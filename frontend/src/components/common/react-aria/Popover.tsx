...
const styles = tv({
  base: 'bg-white shadow-xl rounded-xl border border-gray-200 text-gray-700 outline-0',
  variants: {
    isEntering: {
      true: 'animate-in fade-in placement-bottom:slide-in-from-top-1 placement-top:slide-in-from-bottom-1 placement-left:slide-in-from-right-1 placement-right:slide-in-from-left-1 ease-out duration-150'
    },
    isExiting: {
...
export function Popover({ children, showArrow, className, ...props }: PopoverProps) {
  ...
  return (
    <AriaPopover
      offset={offset}
      {...props}
      className={composeRenderProps(className, (className, renderProps) => styles({
        ...renderProps,
        className 
      }))}>
      {showArrow &&
